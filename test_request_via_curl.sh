...
DATABASE_URL_ENCODED=file%3A%2F%2F%2Fhome%2Fubuntu%2Frecommender-proxy%2Fassets%2FRecommenderDatabase
DATABASE_URL_DECODED=file:///home/ubuntu/recommender-proxy/assets/RecommenderDatabase
OUTPUT_DESTINATION=stdout

curl -k "https://localhost:9002/?datasetEncoded=${DATASET_ENCODED}&datasetAltname=${DATASET_ALTNAME}&assembly=${ASSEMBLY}&stateModel=${STATE_MODEL}&groupEncoded=${GROUP_ENCODED}&groupAltname=${GROUP_ALTNAME}&saliencyLevel=${SALIENCY_LEVEL}&saliencyLevelAltname=${SALIENCY_LEVEL_ALTNAME}&chromosome=${CHROMOSOME}&start=${START}&end=${END}&weightPattern=${WEIGHT_PATTERN}&weightShape=${WEIGHT_SHAPE}&tabixSource=${TABIX_SOURCE}&tabixUrlEncoded=${TABIX_URL_ENCODED}&databaseUrlEncoded=${DATABASE_URL_ENCODED}&outputDestination=${OUTPUT_DESTINATION}"
